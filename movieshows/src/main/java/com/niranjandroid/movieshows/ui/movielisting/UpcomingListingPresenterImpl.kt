...
class UpcomingListingPresenterImpl(private var interactor: ListingContract.Interactor?)
    : BaseListingPresenter(interactor) {

    override fun initMoviesList() {
        compositeDisposable.add(interactor?.fetchUpcomingMovies("1", object : ApiCallBack<MovieListModel> {
    ...
    override fun loadMovies(pageNum: Int) {
        compositeDisposable.add(interactor?.fetchUpcomingMovies(pageNum.toString(), object : ApiCallBack<MovieListModel> {
            override fun onSuccess(movieListModel: MovieListModel) {
