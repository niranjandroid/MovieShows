...
class NowPlayingListingPresenterImpl(private var interactor: ListingContract.Interactor?)
    : BaseListingPresenter(interactor) {

    override fun initMoviesList() {
        compositeDisposable.add(interactor?.fetchNowPlayingMovies("1", object : ApiCallBack<MovieListModel> {
    ...
    override fun loadMovies(pageNum: Int) {
        compositeDisposable.add(interactor?.fetchNowPlayingMovies(pageNum.toString(), object : ApiCallBack<MovieListModel> {
            override fun onSuccess(movieListModel: MovieListModel) {
